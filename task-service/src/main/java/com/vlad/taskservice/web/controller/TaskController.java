...
public class TaskController {
  ...
  public ResponseEntity<TaskResponse> createTask(@RequestBody TaskRequest projectRequest) {
    return ResponseEntity.ok(projectService.createTask(projectRequest));
  }

  @PutMapping("/{id}")
  public ResponseEntity<TaskResponse> updateTask(@PathVariable Long id, @RequestBody TaskRequest projectRequest) {
    return ResponseEntity.ok(projectService.updateTask(id, projectRequest));
  }

  @DeleteMapping("/{id}")
  public ResponseEntity<Boolean> deleteTask(@PathVariable Long id) {
    return ResponseEntity.ok(projectService.deleteTask(id));
  }
}
